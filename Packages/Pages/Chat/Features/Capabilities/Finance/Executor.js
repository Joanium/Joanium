...
export const { handles: handles, execute: execute } = createExecutor({
  ...
  handlers: {
    get_exchange_rate: async (params, onStage) => {
      const { from: from = 'USD', to: to } = params,
      ...
      if (to) {
        ...
        const refs = ['USD', 'EUR', 'GBP', 'JPY', 'INR', 'CAD', 'AUD', 'CHF', 'CNY', 'SGD', 'AED']
          .filter((c) => c !== fromUpper)
          .slice(0, 8)
    ...
    get_treasury_data: async (params, onStage) => {
        ...
        case 'balance':
          ((url = `${BASE}/accounting/dts/dts_table_1/?fields=record_date,open_today_bal,close_today_bal,open_month_bal,open_fiscal_year_bal&sort=-record_date&limit=5`),
            ...
            (formatter = (rows) =>
              rows
                .map((r) => {
                  const open = parseFloat(r.open_today_bal) / 1e3,
                    close = parseFloat(r.close_today_bal) / 1e3;
      ...
      return data.data?.length
        ? [title, '', formatter(data.data), '', 'Source: fiscaldata.treasury.gov'].join('\n')
        : `No treasury data currently available for type "${type}". Try again later.`;
    },
    get_fred_data: async (params, onStage) => {
      const { series_id: series_id, limit: limit = 6 } = params;
      if (!series_id)
        return [
      ...
      const FRED_BASE = 'https://api.stlouisfed.org/fred',
        keyParam = `api_key=${apiKey}&file_type=json`;
      let seriesInfo = null;
      try {
        ...
        const note =
          'abcdefghijklmnopqrstuvwxyz012345' === apiKey
