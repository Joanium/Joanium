const en = {
  appName: 'Joanium',
  composer: {
    placeholder: 'How can I help you today?',
    placeholders: [
      'How can I help you today?',
      'Ask me anything...',
      'What are we working on?',
      'Need help with your code?',
      "Let's build something great.",
      'Type a command or ask a question...',
      "What's on your mind?",
      "What's your next big idea?",
      "Let's solve something interesting.",
      'Design, code, write, or brainstorm...',
      'What would you like to create?',
      "I'm ready. What's the mission?",
    ],
    modelFallback: 'Ready to chat',
    stop: 'Stop',
    generationStopped: 'Generation stopped.',
    attachFiles: 'Attach files',
    enhancePrompt: 'Enhance prompt',
    openTerminal: 'Open terminal',
    startVoiceInput: 'Start voice input',
    stopVoiceInput: 'Stop voice input',
    voiceInputUnsupported: 'Voice input is not available in this version of Chromium.',
    voiceInputPermissionDenied: 'Microphone access was denied. Enable it in your system settings.',
    voiceInputMicrophoneUnavailable: 'No microphone is available. Connect one and try again.',
    voiceInputRecording: 'Listening… Click the microphone again when you are finished.',
    voiceInputTranscribing: 'Transcribing locally…',
    voiceInputDownloadingInitial: 'Preparing local speech model…',
    voiceInputDownloading: 'Downloading local speech model… {progress}%',
    voiceInputNoSpeech: 'No speech was detected. Please try again.',
    voiceInputRecordingFailed: 'Could not record audio. Please try again.',
    voiceInputModelFailed:
      'Could not load the local speech model. Check your connection for the first download.',
    voiceInputStartFailed: 'Could not start voice input. Please try again.',
    contextWindow: 'Context window',
    enhanceFailed: 'Could not enhance prompt.',
    removeAttachmentNamed: 'Remove {name}',
    attachmentRejected: '{count} file(s) could not be attached.',
    attachmentFailed: 'Could not attach files: {message}',
    unknownAttachment: 'Attachment',
    reasoning: 'Reasoning',
    reasoned: 'Reasoned',
    emptyResponse: 'No response received.',
    offlineResponse: 'Offline mode is active. Connect to the internet and try again.',
    networkError: 'Network connection failed. Check your internet connection and try again.',
    localProviderError: 'Local provider is not reachable. Make sure it is running and try again.',
    responseCutOff: "Joanium's response got cut off before the final answer.",
    continueResponse: 'Continue',
    continueHiddenLabel: 'Continue response',
    responseError: 'Unable to get a response right now.',
    workedFor: 'Worked for {duration}',
    privateToggle: 'Toggle private chat',
    scrollToBottom: 'Scroll to bottom',
  },
  diag: {
    statusLabel: 'Status',
    slowResponse: 'Response is taking longer than usual…',
    checkingInternet: 'Checking internet connection…',
    internetStable: 'Internet: Stable',
    internetSlow: 'Internet: Slow',
    internetUnreachable: 'Internet: Unreachable',
    checkingProvider: 'Checking AI provider…',
    providerSlow: 'The AI provider is taking longer than usual to respond',
    providerNoEndpoint: 'AI provider: Status unknown',
    providerSkipped: 'Provider check skipped — internet is unreachable',
    checkSettings: 'Check your API key or provider settings',
  },
  projects: {
    activeHint: 'This project context will be used in new chats.',
    removeActive: 'Remove active project',
  },
  browserPreview: {
    eyebrow: 'Browser',
    title: 'Browser Preview',
    emptyUrl: 'No page loaded',
    ready: 'Ready',
    loading: 'Loading page...',
    back: 'Go back',
    forward: 'Go forward',
    reload: 'Reload',
    close: 'Close preview',
    emptyTitle: 'No browser page yet',
    emptyCopy: 'Open a URL here, or let browser tools show their work in this panel.',
    history: 'History',
    historyTitle: 'Browsing History',
    historyEmpty: 'No browsing history yet.',
    historyClearAll: 'Clear all',
    historyDelete: 'Delete',
  },
  terminal: {
    close: 'Close terminal',
    title: 'Terminal',
    copy: 'Copy',
    copied: 'Copied',
    clear: 'Clear',
    idle: 'Idle',
    running: 'Running',
    finished: 'Finished',
    stopped: 'Stopped',
    noDirectory: 'Choose a working directory first.',
    criticalRisk: 'Critical destructive command blocked.',
    defaultError: 'Command failed.',
    processExited: 'Process exited with code {code}.',
    processStopped: 'Process stopped.',
    completedTool: 'Completed',
    failedTool: 'Failed',
    blockedTool: 'Blocked',
    unsupportedTool: 'Unsupported terminal tool request.',
    projectRequired: 'Open a project before modifying local files.',
    hiddenResultLabel: 'Terminal result',
    resultHeader: 'Terminal tool result',
    outputLabel: 'Output',
    errorLabel: 'Error',
    exitCode: 'Exit code {code}',
    toolLabels: {
      run_shell_command: 'Run command',
      assess_shell_command: 'Assess command',
      inspect_workspace: 'Inspect workspace',
      search_workspace: 'Search workspace',
      read_local_file: 'Read file',
      write_local_file: 'Write file',
      apply_file_patch: 'Patch file',
      delete_local_item: 'Delete item',
      list_directory: 'List directory',
      git_status: 'Git status',
      git_diff: 'Git diff',
      git_branches: 'Git branches',
      git_create_branch: 'Create Git branch',
      git_checkout_branch: 'Checkout Git branch',
      git_delete_branch: 'Delete Git branch',
      git_pull: 'Git pull',
      git_commit: 'Git commit',
      git_push: 'Git push',
      git_push_sync: 'Git pull and push',
      run_project_checks: 'Run project checks',
      start_local_server: 'Start local server',
      read_terminal_output: 'Read terminal output',
    },
  },
  tools: {
    completedTool: 'Completed',
    failedTool: 'Failed.',
    hiddenResultLabel: 'Tool result',
    subAgentsLabel: 'Sub-agents',
    subAgentsRunning: 'Running sub-agents...',
    subAgentsComplete: 'Sub-agents completed.',
    subAgentsFailed: 'Sub-agents failed.',
    subAgentsResultHeader: 'Sub-agent handoffs',
    subAgentsCoordinationGoal: 'Coordination goal: {goal}',
    subAgentsNoTasks: 'No valid sub-agent tasks were provided.',
    subAgentsTaskHeader: '{index}. {title}',
    subAgentsGoal: 'Goal: {goal}',
    subAgentsDeliverable: 'Expected handoff: {deliverable}',
    subAgentsHandoff: 'Handoff',
    subAgentsError: 'Error',
    subAgentsCountProgress: '{done} of {total} done',
    subAgentStatusQueued: 'Queued',
    subAgentStatusRunning: 'Working',
    subAgentStatusDone: 'Done',
    subAgentStatusFailed: 'Failed',
    subAgentTaskSection: 'Task',
    subAgentPromptSection: 'Prompt',
    subAgentOutputSection: 'Output',
    subAgentErrorSection: 'Error',
    subAgentDepthLimit:
      'Task reached the tool call limit before completing. Returning partial progress.',
  },
  git: {
    refresh: 'Refresh git status',
    unknownBranch: 'unknown',
    dirty: 'Uncommitted changes',
    clean: 'Clean',
    ahead: '{count} ahead',
    behind: '{count} behind',
    noProjectFolder: 'No project folder is set for this project.',
    refreshFailed: 'Could not refresh git status.',
    actionComplete: 'Git action completed.',
    actionFailed: 'Git action failed.',
    switchBranch: 'Switch branch from {branch}',
    createBranch: 'Create branch',
    createBranchPlaceholder: 'New branch name',
    loadingBranches: 'Loading branches...',
    branchesLoadFailed: 'Could not load branches.',
    noBranches: 'No branches found.',
    branchCreated: 'Created and switched to {branch}.',
    branchSwitched: 'Switched to {branch}.',
    deleteBranch: 'Delete branch',
    branchDeleted: 'Deleted branch {branch}.',
    cannotDeleteCurrent: 'Cannot delete the currently checked-out branch.',
    cannotDeleteProtected: '"{branch}" is a protected branch and cannot be deleted.',
    emptyOutput: 'No output.',
    actions: {
      diff: 'View diff',
      push: 'Push',
      pull: 'Pull',
      status: 'Status',
      sync: 'Sync',
      commit: 'Commit',
      commitPush: 'Commit & Push',
      commitSync: 'Commit & Sync',
    },
    actionMeta: {
      push: 'Push local commits',
      pull: 'Pull remote changes',
      sync: 'Pull, then push',
      commit: 'Stage files and create a commit',
      commitPush: 'Commit, then push',
      commitSync: 'Commit, pull, then push',
    },
    resultLabels: {
      diff: 'Git diff',
      push: 'Git push',
      pull: 'Git pull',
      status: 'Git status',
      sync: 'Git sync',
      commit: 'Git commit',
      checkout: 'Git checkout',
      createBranch: 'Git branch',
      deleteBranch: 'Git branch delete',
    },
    commands: {
      diff: 'git diff',
      push: 'git push',
      pull: 'git pull',
      status: 'git status',
      sync: 'git pull && git push',
      commit: 'git commit',
      checkout: 'git checkout',
      createBranch: 'git checkout -b',
      deleteBranch: 'git branch -d',
    },
    moreActions: 'More git actions',
    cancel: 'Cancel',
    commitMessagePlaceholder: 'Commit message',
    commitMessageRequired: 'Enter a commit message first.',
    generateCommitMessage: 'Generate commit message',
    noDiffForCommitMessage: 'No changes found to summarize.',
    diffTruncated: '(diff truncated for commit message generation)',
    aiCommitEmpty: 'AI returned an empty commit message.',
    aiCommitFailed: 'Could not generate a commit message.',
    commitStatus: {
      generating: 'Generating message...',
      staging: 'Staging files...',
      hooks: 'Running commit hooks...',
      pushing: 'Pushing...',
      syncing: 'Syncing...',
    },
  },
  memorySync: {
    updating: 'Updating memory...',
    catchingUp: 'Catching up memory...',
    catalogLabel: 'Current memory catalog',
    transcriptLabel: 'Conversation to review',
    availableFiles: 'Available files: {files}',
    fileHeader: '--- {filename} ---',
    contentLabel: 'Content:',
    emptyContent: '(empty)',
    userLabel: 'User',
    assistantLabel: 'Assistant',
    attachmentsLabel: 'Attachments: {attachments}',
    noText: '(no text)',
  },
  dropZone: {
    label: 'Drop files here',
    labelWithImages: 'Drop files or images here',
    hint: 'Supported: documents, code, and text',
    hintWithImages: 'Supported: documents, code, text, and images',
  },
  techFeed: {
    label: 'Tech feed',
    hn: 'Hacker News',
    github: 'Trending on GitHub',
    dev: 'DEV Community',
    redditProgramming: 'Reddit /r/programming',
    redditMachineLearning: 'Reddit /r/MachineLearning',
    lobsters: 'Lobste.rs',
    stackOverflow: 'Stack Overflow',
    codeFallback: 'Code',
    authorFallback: 'someone',
    postedBy: 'Posted by {name}',
    meta: {
      separator: ' - ',
      points: '{count} points',
      comments: '{count} comments',
      stars: '{count} stars',
      reactions: '{count} reactions',
      readTime: '{count} min read',
      votes: '{count} votes',
      answers: '{count} answers',
      views: '{count} views',
    },
    empty: 'No news available right now.',
  },
  slash: {
    label: 'Slash commands',
    sections: {
      actions: 'Actions',
      modes: 'Modes',
      navigate: 'Navigate',
      templates: 'Templates',
      agents: 'Agents',
    },
    badges: {
      action: 'Action',
      mode: 'Mode',
      navigate: 'Nav',
      template: 'Template',
      agent: 'Agent',
    },
  },
  whatsNew: {
    badge: 'New',
    heading: "What's New",
  },
  modelInfo: {
    infoButton: 'Model info',
    contextWindow: 'Context',
    maxOutput: 'Max output',
    pricing: 'Pricing',
    inputPrice: 'Input',
    outputPrice: 'Output',
    perMillion: '/1M tokens',
    supportsText: 'Text',
    supportsImage: 'Images',
    supportsPdf: 'PDF',
    supportsDocx: 'Documents',
    thinking: 'Extended thinking',
  },
};

export default en;
